# 11.3 - Modelado del Sistema — Modelo Entidad Relación (MER)
...
El diseño usa **14 tablas** (4 de identidad con herencia + 5 de documentación + 5 de ambulancias). En la versión anterior se usaba una sola tabla `USUARIO` con un discriminador `tipo`; ahora se normalizó siguiendo la herencia del MER: `USUARIO` como base y `FUNCIONARIO`/`PACIENTE` como subtablas. Además se simplificó el modelo de carga: en vez de crear una tabla por cada tipo de elemento trasladado, usamos una sola tabla `ELEMENTO_TRASLADO` con un campo `tipo` que discrimina qué es (paciente, órgano, equipo o insumo).
...
El hospital tiene dos tipos de personas, modelados con herencia **table-per-class**:

- `USUARIO` → tabla base con datos comunes (nombre, email, documento)
- `FUNCIONARIO` → extiende USUARIO, agrega credenciales de login y rol
- `PACIENTE` → extiende USUARIO, agrega token de acceso por QR

| Tipo | ¿Qué hace? | Campos que usa | Tabla |
|---|---|---|---|
| **FUNCIONARIO** | Entra con usuario y contraseña. Puede ser admin, superadmin o conductor. Tiene licencia de conducir. | username, password_hash, licencia, telefono, rol | funcionario |
| **PACIENTE** | No necesita login. Accede a documentos por QR. Se registra cuando viaja en ambulancia. | token_acceso | paciente |
...
```
USUARIO                                         ← base: datos comunes
├── id: INT (PK, AUTO_INCREMENT)
├── tipo: ENUM('funcionario', 'paciente')
├── nombre, apellido, email
├── documento_identidad
└── created_at

FUNCIONARIO                                     ← extiende USUARIO (login)
├── id: INT (PK, FK → USUARIO)
├── username, password_hash
├── licencia, telefono
├── activo: BOOLEAN
└── rol: ENUM('admin', 'superadmin', 'conductor')

CODIGO_QR                                       ← códigos QR para documentos
├── id: INT (PK, AUTO_INCREMENT)
├── nombre: VARCHAR(100) (UNIQUE)
└── descripcion: TEXT

PACIENTE                                        ← extiende USUARIO (QR)
├── id: INT (PK, FK → USUARIO)
├── token_acceso
└── codigo_qr_id: INT (FK → CODIGO_QR)         ← QR que escanea
```
...
├── archivo_path, archivo_nombre, codigo_qr_id (FK → CODIGO_QR), qr_path
...
## Resumen rápido (14 tablas)

| # | Tabla | Módulo | ¿Qué guarda? |
|---|---|---|---|
| 1 | USUARIO | Identidad | Base compartida con datos comunes de toda persona |
...
| 4 | CODIGO_QR | Identidad | Códigos QR que los pacientes escanean |
...
DOCUMENTO (1) ──── (1) CODIGO_QR
  · Un documento tiene un código QR asociado.

CODIGO_QR (1) ──── (N) PACIENTE
  · Un código QR es escaneado por muchos pacientes.
  · PACIENTE (N) ──── (1) CODIGO_QR — "escanea"
```
