...
> **Estado real (Jul 2026):** Sprint 0 ✅. Frontend: ~80% implementado (layout, dashboard, docs, encuestas, traslados, perfil, pública). Backend: Domain layer completo, Infrastructure parcial (faltan 2 repos, 2 servicios), Application layer **vacío** (solo `.gitkeep`). Controladores `Conductor` y `Ruta` son stubs. `TrasladoController` usa datos mock. Sin tests.
...
**Objetivo:** Completar la arquitectura hexagonal: crear la capa Application (Use Cases) que está vacía, implementar los repos y servicios faltantes, reemplazar datos mock por consultas reales a BD, y agregar los FR de identidad pendientes (FR-03, FR-06).

**Estado actual del backend:**
- `src/Application/UseCases/` — **VACÍO** (solo `.gitkeep`)
- `src/Infrastructure/Persistence/MySQL/` — Faltan: `TrasladoRepository`, `ConductorRepository`
- `src/Infrastructure/Service/` — Faltan: `QRGeneratorService`, `FileStorageService`
- `ConductorController` — **STUB** (18 líneas, renderiza vista vacía)
- `RutaController` — **STUB** (18 líneas, renderiza vista vacía)
- `TrasladoController` — usa datos mock, no BD real
- `views/conductores/` — "En construcción"
- `views/rutas/` — "En construcción"
...
| B1.19 | QR Service: generar QR con `chillerlan/php-qrcode` — `QRGeneratorService` | Composer | `src/Infrastructure/Service/QRGeneratorService.php` |
...
| B1.22 | Refactor `TrasladoController`: reemplazar datos mock por use cases reales | B1.7–B1.11 | `src/Infrastructure/Web/Controller/TrasladoController.php` |
...
| B1.24 | Refactor `ConductorController`, `RutaController`: reemplazar stubs por lógica real con use cases | B1.13–B1.14 | `src/Infrastructure/Web/Controller/*.php` |
| B1.25 | Endpoint público: servir PDF por token QR (sin auth) | B1.20 | `PublicController::verDocumento()` |
| B1.26 | Use Case: `SolicitarResetPasswordUseCase` (generar token, email con link) — FR-03 (Should) | — | `src/Application/UseCases/Auth/SolicitarResetPasswordUseCase.php` |
| B1.27 | Use Case: `EjecutarResetPasswordUseCase` (validar token, actualizar password) — FR-03 (Should) | B1.26 | `src/Application/UseCases/Auth/EjecutarResetPasswordUseCase.php` |
| B1.28 | Vistas pública: formulario solicitar reset + formulario nuevo password | B1.26–B1.27 | `views/auth/recuperar.php`, `views/auth/reset-password.php` |
| B1.29 | Al registrar traslado con elemento tipo `paciente`, crear registro en `PACIENTE` si no existe (CI como matching) — FR-06 (Should) | T.4 | `RegistrarTrasladoUseCase` |

### Frontend

| # | Tarea | FR / MoSCoW | Archivos destino |
|---|-------|-------------|------------------|
| F1.1 | Reemplazar vista "En construcción" de conductores con listado real + formulario crear/editar | FR-04 (Must) | `views/conductores/` |
| F1.2 | Reemplazar vista "En construcción" de rutas con listado real + formulario crear/editar | FR-23 (Should) | `views/rutas/` |
| F1.3 | Vista pública formulario solicitar reset + nuevo password (estilo Web 2.0) | FR-03 (Should) | `views/auth/recuperar.php`, `views/auth/reset-password.php` |
...
## Sprint 2 — Gestión de Usuarios y Refinamiento Frontend (1-2 sem)
...
- `views/funcionarios/` ❌ **no existe** — hay que crearlo
...
| F2.4 | Dashboard paciente: refinar stat cards, avatar con iniciales, navegación por pestañas (Docs/Encuestas/Traslados) | FR-06 (Should) | `views/dashboard/paciente.php` |
| F2.5 | Perfil: refinar con diseño Web 2.0, foto con hover para cambiar, secciones plegables | FR-07 (Could) | `views/perfil/index.php` |
...
**Objetivo:** Calendario de traslados, mapa de rutas, búsqueda global. Todo con estética Web 2.0 retro / Old Facebook.
...
| F4.2 | Mapa de rutas con Leaflet + OpenStreetMap: origen/destino/paradas en mapa interactivo | `views/traslados/mapa.php`, `views/rutas/index.php` |
...
| B7.3 | Tests de seguridad: CSRF bypass, SQL injection, XSS, path traversal | B7.1 | SEG-04, SEG-05, SEG-06 |
| B7.4 | Auditoría OWASP Top 10 (lista de verificación) + mapeo contra controles C-01 a C-15 | — | Todas |
| B7.5 | **Bloqueo por intentos fallidos**: 5 intentos → cuenta bloqueada 15 min (extiende rate limiter) | — | R-P-01 |
| B7.6 | **Forzar HTTPS**: redirect HTTP→HTTPS + HSTS preload | — | R-P-02, R-P-03, SEG-07 |
| B7.7 | **Límite de sesiones concurrentes** por usuario (máx 1) | — | R-P-08 |
| B7.8 | **Auditoría de acceso a datos**: registrar qué funcionario consultó qué paciente/documento | — | R-P-06 |
| B7.9 | **Baja lógica de usuarios**: desactivar cuenta (derecho cancelación Ley 18.331 Art. 15) | — | R-P-11 |
| B7.10 | **Fail2ban config**: reglas para bloqueo automático de IPs maliciosas | — | R-P-14 |
...
| B7.13 | Documentación técnica: README actualizado, setup instructions, manual deploy | — | — |
| B7.14 | **Registro de bases de datos ante URCDP**: preparar documentación necesaria | — | R-P-04, Ley 18.331 Art. 28 |
...
| **1** — Capa Aplicación, Repos, Servicios | 3-4 | 29 | 3 | 32 | ◐ Use Cases vacíos, 2 repos faltan, 2 services faltan, 2 ctrls stub |
| **2** — Gestión Usuarios & Refinamiento | 1-2 | 4 | 5 | 9 | ◐ Paciente dashboard/perfil ya existen, falta funcionarios |
| **3** — Reportes & Gráficos | 1-2 | — | 6 | 6 | ○ No empezado |
| **4** — UX Avanzado | 1-2 | — | 6 | 6 | ○ No empezado |
| **5** — PWA & Mobile | 1 | — | 6 | 6 | ○ No empezado |
| **6** — Polish & Accesibilidad | 1 | — | 7 | 7 | ○ No empezado |
| **7** — Testing, Auditoría & Seg. Prod. | 2-3 | 14 | 5 | 19 | ○ No empezado |
| **Total** | **10-15 sem** | **60** | **43** | **103** | |
...
|---------|----------------------|----------|
| **Application Layer** | Sprint 1 "en progreso" | ❌ Vacío (solo `.gitkeep`) |
| **TrasladoController** | Sprint 1 listo | ◐ Usa datos mock, no BD |
| **ConductorController** | Sprint 1 listo | ❌ Stub de 18 líneas |
| **RutaController** | Sprint 1 listo | ❌ Stub de 18 líneas |
| **QRGeneratorService** | Sprint 1 listo | ❌ QR solo en JS cliente |
| **FileStorageService** | Sprint 1 listo | ❌ Lógica inline en controller |
| **Dashboard paciente** | Sprint 2 pendiente | ✅ Ya existe |
| **Perfil** | Sprint 2 pendiente | ✅ Ya existe con foto |
| **Vista pública QR** | Sprint 2 pendiente | ✅ Ya existe |
| **Estilo visual** | "Windows clásico" | Web 2.0 retro (Old Facebook) |
| **CSS** | `classic.css` (no existe) | `web20.css` (1314 líneas, real) |
| **Tests** | Sprint 7 | ❌ Vacío |
