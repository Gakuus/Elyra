...
> **Estado real (Jul 2026):** Sprint 0 ✅. Frontend: ~85% implementado (layout, dashboard, docs, encuestas, traslados, perfil, pública, conductores, rutas, reset password). Backend: Domain layer completo (15 entidades, 9 interfaces, 6 VOs), Infrastructure **sólida** (9 repos MySQL, 7 servicios, todos con implementación real). Application layer **completo** (22 use cases en 6 módulos). Todos los controllers refactorizados a use cases. Tests: 34 unit + 2 integración + 19 escenarios BDD (13 pasan, 6 fallan por módulos no implementados).
...
- `UsuarioRepository` ✅ tiene CRUD de funcionarios (546 líneas, `findAllFuncionarios`, `saveFuncionario`, etc.)
- `FuncionarioController` ❌ no existe
- Rutas `/funcionarios` ❌ no definidas en `web.php`
...
- `tests/Unit/` — 34 archivos: entidades (13), value objects (6), repos (4), servicios (4), controllers (0 vacío), use cases (0 vacío)
- `tests/Integration/` — 2 archivos: `TrasladoStateMachineTest`, `DocumentFlowTest`
- `tests/Behat/` — 19 escenarios BDD en español (13 pasan, 6 fallan por módulos no implementados)
- `phpunit.xml.dist` ✅ configurado (Unit + Integration suites)
...
| # | Tarea | Estado | Notas |
|---|-------|--------|-------|
| B7.1 | Tests unitarios: Value Objects, Domain Services, Use Cases | ◐ | VOs ✅ (6), Entidades ✅ (13), Repos ✅ (4), Servicios ✅ (4). Use Cases vacío (no hay use cases). Controller tests vacío. |
...
| B7.9 | **Baja lógica de usuarios**: desactivar cuenta | ◐ | `Funcionario.setActivo()/isActivo()` existe. `AuthService` verifica `isActivo()`. Falta UI y controller. |
...
| **2** — Gestión Usuarios & Refinamiento | 1-2 | 4 | 5 | 9 | ✅ Use Cases ✅, FuncionarioController ✅, rutas ✅, vistas ✅ (listado, crear/editar, modal desactivar). Dashboard paciente y perfil ya existen. F2.4/F2.5 (pulido) pendientes. |
...
| **7** — Testing, Auditoría & Seg. Prod. | 2-3 | 14 | 5 | 19 | ◐ 34 unit tests ✅ (entidades, VOs, repos, servicios), 2 integración ✅, 19 escenarios BDD ✅ (13 pasan, 6 fallan). Falta: controller tests, use case tests, security tests, Docker, HTTPS, fail2ban, deploy |
